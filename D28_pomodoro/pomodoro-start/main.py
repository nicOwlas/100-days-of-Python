...
def main():
    ...
    def reset_timer():
        ...
        timer_label.config(
            text="Timer", font=(FONT_NAME, 60, "normal"), fg=GREEN, bg=YELLOW
        )
    ...
    # Absolute path as packaging with pants does not work with relative paths unfortunately
    tomato_image = PhotoImage(
        file="/Users/nicolas/git/100-days-of-Python/D28_pomodoro/pomodoro-start/tomato.png"
    )
    canvas.create_image(120, 120, image=tomato_image)
    timer_text = canvas.create_text(
        120,
        145,
        text=f"{count_minutes:02}:{count_seconds:02}",
